...
* added the posibility to add a class name to the container through container-class property
...
```bash
bower install angucomplete-alt2 --save
```

Or

```bash
npm install @thenablyn/angucomplete-alt2 --save
```

Then add the angucomplete-alt module to your Angular App file, e.g.

```js
let app = angular.module('app', ["angucomplete-alt"]);
```
...
When the field is required and the value is not valid, the class ```angucomplete-error``` is added to container so you can customize style wit that.
